...
async function loadChannelEPG(channel) {
    try {
        const response = await fetch(
            `${API_BASE}/live-tv/channels/${channel.id}/programs`
        );
        const data = await response.json();
        ...
        if (data.success && data.data) {
            channel.programs = data.data.map((program) => ({
                ...
                start: new Date(program.start_time).toLocaleTimeString(
                    "ar-SA",
                    { hour: "2-digit", minute: "2-digit", hour12: false }
                ),
                end: new Date(program.end_time).toLocaleTimeString("ar-SA", {
                    hour: "2-digit",
                    minute: "2-digit",
                    hour12: false,
                }),
...
function renderChannels() {
    ...
    if (!channelsData || channelsData.length === 0) {
        container.html(
            '<div class="text-center text-gray-400 py-10">لا توجد قنوات متاحة</div>'
        );
        return;
    }
    ...
    channelsData.forEach((channel) => {
        ...
        if (
            activeCategory &&
            activeCategory !== "all" &&
            channel.category_id != activeCategory
        ) {
            return;
        }
        ...
        const watchButton =
            channel.stream_health_status === "online"
                ? `
    ...
    `
        ...
        const channelHtml = `
                    <div class="channel-card rounded-lg p-4" data-category="${
                        channel.category_id
                    }" data-channel-id="${channel.id}">
                        <div class="flex gap-4 items-center mb-4">
                            <img src="${
                                channel.logo_url || "/imgs/default-channel.png"
                            }" alt="${
            channel.name_ar
        }" class="w-12 h-12 rounded-full object-cover" onerror="this.src='/imgs/default-channel.png'">
                            <div>
                        ...
                        <div class="relative p-4 rounded-lg timeline-container">
                            ...
                            <div class="flex overflow-x-auto gap-2 pb-2 scroll-container">
                                ...
                                `
                                              ...
                                              .join("")
                                        : '<div class="text-sm text-gray-400">لا توجد برامج متاحة</div>'
                                }
                            </div>
                        </div>
                    </div>
...
function showError(message) {
    $("#channelsContainer").html(
        `<div class="text-center text-red-400 py-10">${message}</div>`
    );
}
...
function setupEventListeners() {
    ...
    $("#closeLiveStream, #liveStreamModal").on("click", function (e) {
        if (
            e.target === this ||
            $(e.target).closest("#closeLiveStream").length
        ) {
...
function showProgramModal(program, channel) {
    ...
    $("#modalContent").html(`
    <div class="flex gap-4 items-center mb-4">
      <img src="${channel.logo_url || "/imgs/default-channel.png"}" alt="${
        channel.name_ar
    }" 
...
async function showLiveStream(channelId) {
    ...
    try {
        ...
        const response = await fetch(
            `${API_BASE}/live-tv/channels/${channelId}/stream`
        );
        const data = await response.json();
    ...
    $(".channel-card").each(function () {
        ...
        if (currentProgram.length > 0 && scrollContainer.length > 0) {
            ...
            const scrollPosition =
                programPosition - containerWidth / 2 + programWidth / 2;
    ...
    if (muteBtn) {
        muteBtn.addEventListener("click", function () {
            if (video.muted) {
                ...
                muteBtn.innerHTML =
                    '<svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"></path></svg>';
            } else {
                video.muted = true;
