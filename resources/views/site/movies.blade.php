<x-front-layout>
    ...
    <section id="hero" class="overflow-hidden relative h-screen">
        <!-- Slides -->
        @foreach ($moviesHero as $movie)
            <div class="absolute inset-0 opacity-100 hero-slide">
                <img src="{{ $movie->backdrop_full_url }}" alt="Hero Background"
                    class="object-cover absolute inset-0 w-full h-full" />
                <video src="{{ $movie->trailer_full_url }}" class="hidden object-cover absolute inset-0 w-full h-full"
                    playsinline></video>
            ...
            @foreach ($moviesHero as $movie)
                <button
                    class="w-full h-[72px] hero-dot opacity-75 hover:opacity-100 hover:scale-110 transition-all duration-300"><img
                        src="{{ $movie->poster_full_url }}" alt="logo1"
    ...
    @push('scripts')
        <script>
            $(document).ready(function() {
                ...
                function renderSection(section,indexSection) {
                    ...
                    $.ajax({
                        url: "{{ route('site.movie.get-html-section') }}",
                        method: 'GET',
                        data: {
        ...
        <script>
            const contents = @json($moviesHeroArray);
        </script>
    @endpush
