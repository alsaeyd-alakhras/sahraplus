<x-dashboard-layout>
    ...
    <x-slot:extra_nav>
        ...
        @can('create', 'App\\Models\Tax')
            <div class="mx-2 nav-item">
                <a href="{{ route('dashboard.taxes.create') }}" class="m-0 btn btn-icon text-success">
    ...
    @php
        $fields = [
            ...
            'tax_code' => __('admin.tax_code'),
            'tax_rate' => __('admin.tax_rate'),
            'effective_from' => __('admin.effective_from'),
            'tax_type' => __('admin.tax_type'),
    ...
    @push('scripts')
        ...
        <script>
            ...
            const urlIndex = `{{ route('dashboard.taxes.index') }}`;
            const urlFilters = `{{ route('dashboard.taxes.filters', ':column') }}`;
            const urlCreate = '{{ route('dashboard.taxes.create') }}';
            const urlShow = '{{ route('dashboard.taxes.show', ':id') }}';
            const urlEdit = '{{ route('dashboard.taxes.edit', ':id') }}';
            const urlDelete = '{{ route('dashboard.taxes.destroy', ':id') }}';

            // ability
            const abilityCreate = "{{ Auth::guard('admin')->user()->can('create', 'App\\Models\\Tax') }}";
            const abilityShow = "{{ Auth::guard('admin')->user()->can('show', 'App\\Models\\Tax') }}";
            const abilityEdit = "{{ Auth::guard('admin')->user()->can('update', 'App\\Models\\Tax') }}";
            const abilityDelete = "{{ Auth::guard('admin')->user()->can('delete', 'App\\Models\\Tax') }}";
