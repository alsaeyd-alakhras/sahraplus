<x-dashboard-layout>
    ...
    <x-slot:extra_nav>
        ...
        <div class="mx-2 nav-item">
            <a href="{{ route('dashboard.series.create') }}" class="m-0 btn btn-icon text-success">
        ...
        <div class="enhanced-card-body">
            <div class="col-12" style="padding: 0;">
                <div class="table-container">
                    <table id="series-table" class="table enhanced-sticky table-striped table-hover" style="display: table; width:100%; height: auto;">
                        <thead>
                            <tr>
    ...
    @push('scripts')
        ...
        <script>
            ...
            const urlIndex   = `{{ route('dashboard.series.index') }}`;
            const urlFilters = `{{ route('dashboard.series.filters', ':column') }}`; // ← صححنا اسم الراوت
            const urlCreate  = '{{ route("dashboard.series.create") }}';
            const urlShow    = '{{ route("dashboard.series.show", ":id") }}';
            const urlEdit    = '{{ route("dashboard.series.edit", ":id") }}';
            const urlDelete  = '{{ route("dashboard.series.destroy", ":id") }}';

            // ability
            const abilityCreate = "{{ Auth::guard('admin')->user()->can('create', 'App\\Models\\Series') }}";
            const abilityShow   = "{{ Auth::guard('admin')->user()->can('show',   'App\\Models\\Series') }}";
            const abilityEdit   = "{{ Auth::guard('admin')->user()->can('update', 'App\\Models\\Series') }}";
            const abilityDelete = "{{ Auth::guard('admin')->user()->can('delete', 'App\\Models\\Series') }}";
