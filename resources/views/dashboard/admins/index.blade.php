<x-dashboard-layout>
    ...
    <x-slot:extra_nav>
        ...
        <div class="mx-2 nav-item">
            <a href="{{ route('dashboard.admins.create') }}" class="m-0 btn btn-icon text-success">
        ...
        <div class="enhanced-card-body">
            <div class="col-12" style="padding: 0;">
                <div class="table-container">
                    <table id="admins-table" class="table enhanced-sticky table-striped table-hover"style="display: table; width:100%; height: auto;">
                        <thead>
                            <tr>
    ...
    @push('scripts')
        ...
        <script>
            ...
            const urlIndex = `{{ route('dashboard.admins.index') }}`;
            const urlFilters = `{{ route('dashboard.admins.filters', ':column') }}`;
            const urlCreate = '{{ route("dashboard.admins.create") }}';
            const urlShow = '{{ route("dashboard.admins.show", ":id") }}';
            const urlEdit = '{{ route("dashboard.admins.edit", ":id") }}';
            const urlDelete = '{{ route("dashboard.admins.destroy", ":id") }}';

            // ability
            const abilityCreate = "{{ Auth::guard('admin')->user()->can('create', 'App\\Models\\Admin') }}";
            const abilityShow = "{{ Auth::guard('admin')->user()->can('show', 'App\\Models\\Admin') }}";
            const abilityEdit = "{{ Auth::guard('admin')->user()->can('update', 'App\\Models\\Admin') }}";
            const abilityDelete = "{{ Auth::guard('admin')->user()->can('delete', 'App\\Models\\Admin') }}";
            ...
            const columnsTable = [
                ...
                { data: 'status', name: 'status', orderable: false, class: 'text-center', render: function(data, type, row) {
                    const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000) // 5 دقائق
                    if (new Date(data) >= fiveMinutesAgo) {
                        return `
                            <span style="
                                display:inline-flex;
                                align-items:center;
