...
<div class="p-3 mb-3 rounded border shadow-sm country-row card">
    ...
    <div class="row g-3 align-items-end">
        ...
        <div class="col-md-3">
            ...
            <select name="countryPrices[{{ $i }}][country_id]" class="form-select country-select"
                data-index="{{ $i }}">
                <option value=""> {{ __('admin.select_country') }}</option>
                @foreach ($countries as $country)
                    <option value="{{ $country->id }}" @if (isset($row['country_id']) && $row['country_id'] == $country->id) selected @endif>
        ...
        <div class="col-md-3">
            ...
            <input type="text" name="countryPrices[{{ $i }}][currency]"
                class="form-control currency-input" value="{{ $row['currency'] ?? '' }}">
        </div>
        ...
        $(document).on('change', '.country-select', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('input[name="countryPrices[' + rowIndex + '][currency]"]').val(response.currency);
                }
            });
        });
    </script>
