<x-dashboard-layout>
    ...
    <x-slot:extra_nav>
        ...
        @can('create', 'App\\Models\\SubscriptionPlan')
            <div class="mx-2 nav-item">
                <a href="{{ route('dashboard.sub_plans.create') }}" class="m-0 btn btn-icon text-success">
    ...
    @php
        ...
        $fields = [
            ...
            'video_quality' => __('admin.video_quality'),
            'billing_period' => __('admin.billing_period'),
    ...
    @push('scripts')
        ...
        <script>
            ...
            const abilityShow = "{{ Auth::guard('admin')->user()->can('show', 'App\\Models\\SubscriptionPlan') }}";
            const abilityEdit = "{{ Auth::guard('admin')->user()->can('update', 'App\\Models\\SubscriptionPlan') }}";
            const abilityDelete = "{{ Auth::guard('admin')->user()->can('delete', 'App\\Models\\SubscriptionPlan') }}";

            // أسماء الحقول للفلترة في الهيدر
            const fields = ['#', 'name_ar', 'name_en', 'price', 'currency', 'video_quality', 'billing_period', 'sort_order',
                'is_active'
            ];
            ...
            const columnsTable = [{
                ...
                // العمليات
                {
                    ...
                    render: function(data, type, row) {
                        ...
                        if (abilityEdit) {
                            linkedit =
                                `<a href="${urlEdit.replace(':id', data)}" class="action-btn btn-edit" title="تعديل"><i class="fas fa-edit"></i></a>`;
                        }
                        if (abilityDelete) {
                            linkdelete =
                                `<button class="action-btn btn-delete delete_row" data-id="${data}" title="حذف"><i class="fas fa-trash"></i></button>`;
                        }
