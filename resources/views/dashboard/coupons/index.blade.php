<x-dashboard-layout>
    ...
    <x-slot:extra_nav>
        ...
        @can('create', 'App\\Models\Coupon')
            <div class="mx-2 nav-item">
                <a href="{{ route('dashboard.coupons.create') }}" class="m-0 btn btn-icon text-success">
    ...
    @php
        $fields = [
            ...
            'discount_value' => __('admin.discount_value'),
            'starts_at' => __('admin.starts'),
            'expires_at' => __('admin.expires_at'),
            'usage_limit' => __('admin.usage_limit'),
    ...
    @push('scripts')
        ...
        <script>
            ...
            const urlIndex = `{{ route('dashboard.coupons.index') }}`;
            const urlFilters = `{{ route('dashboard.coupons.filters', ':column') }}`;
            const urlCreate = '{{ route('dashboard.coupons.create') }}';
            const urlShow = '{{ route('dashboard.coupons.show', ':id') }}';
            const urlEdit = '{{ route('dashboard.coupons.edit', ':id') }}';
            const urlDelete = '{{ route('dashboard.coupons.destroy', ':id') }}';

            // ability
            const abilityCreate = "{{ Auth::guard('admin')->user()->can('create', 'App\\Models\\Coupon') }}";
            const abilityShow = "{{ Auth::guard('admin')->user()->can('show', 'App\\Models\\Coupon') }}";
            const abilityEdit = "{{ Auth::guard('admin')->user()->can('update', 'App\\Models\\Coupon') }}";
            const abilityDelete = "{{ Auth::guard('admin')->user()->can('delete', 'App\\Models\\Coupon') }}";
