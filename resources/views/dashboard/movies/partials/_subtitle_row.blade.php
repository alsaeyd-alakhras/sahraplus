@php
    ...
    $isLocal = $url && Str::startsWith($url, ['/storage/', asset('storage')]);
...
<div class="p-3 rounded border sub-row card">
    <div class="row g-3 align-items-end">
        <input type="hidden" name="subtitles[{{ $i }}][id]" class="sub-id" value="{{ $id_sub }}">
        ...
        <div class="col-md-3">
            ...
            <select name="subtitles[{{ $i }}][language]" class="form-select sub-language">
                @foreach (['ar' => 'العربية', 'en' => 'English', 'fr' => 'Français', 'es' => 'Español', 'de' => 'Deutsch', 'it' => 'Italiano', 'tr' => 'Türkçe', 'fa' => 'فارسی', 'ur' => 'اردو', 'ru' => 'Русский', 'zh' => '中文', 'ja' => '日本語', 'ko' => '한국어'] as $code => $txt)
                    <option value="{{ $code }}" {{ $lang == $code ? 'selected' : '' }}>{{ $txt }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="col-md-3">
            ...
            <input type="text" class="form-control sub-label" name="subtitles[{{ $i }}][label]"
                value="{{ $label }}" placeholder="{{ __('admin.label_placeholder') }}">
        ...
        <div class="col-md-6">
            ...
            <div class="d-flex gap-3 mb-2">
                <div class="form-check form-check-inline">
                    <input class="form-check-input sub-source" type="radio"
                        name="subtitles[{{ $i }}][source_type]" value="file"
                        id="sub-src-file-{{ $i }}" {{ $source == 'file' ? 'checked' : '' }}>
                    <label class="form-check-label"
                        for="sub-src-file-{{ $i }}">{{ __('admin.upload_file') }}</label>
                </div>
                <div class="form-check form-check-inline">
                    <input class="form-check-input sub-source" type="radio"
                        name="subtitles[{{ $i }}][source_type]" value="url"
                        id="sub-src-url-{{ $i }}" {{ $source == 'url' ? 'checked' : '' }}>
                    <label class="form-check-label"
                        for="sub-src-url-{{ $i }}">{{ __('admin.external_url') }}</label>
                </div>
            </div>
            <input type="text" class="form-control mb-2 sub-url {{ $source == 'file' ? 'd-none' : '' }}"
                name="subtitles[{{ $i }}][url]" value="{{ $url }}"
            ...
            <input type="hidden" name="subtitles[{{ $i }}][existing_url]"
        ...
        <div class="col-12 d-flex justify-content-between align-items-center">
            <div class="form-check">
                <input class="form-check-input sub-default" type="checkbox"
                    name="subtitles[{{ $i }}][is_default]" value="1"
                    {{ $isDefault ? 'checked' : '' }}>
