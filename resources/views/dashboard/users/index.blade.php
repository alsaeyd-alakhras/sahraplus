<x-dashboard-layout>
    ...
    <x-slot:extra_nav>
        ...
        <div class="mx-2 nav-item">
            <a href="{{ route('dashboard.users.create') }}" class="m-0 btn btn-icon text-success">
    ...
    <div class="shadow-lg enhanced-card">
        <div class="table-header-title">
            <i class="icon ph ph-users me-2"></i>
            جدول المستخدمين
        </div>
        <div class="enhanced-card-body">
            <div class="col-12" style="padding: 0;">
                <div class="table-container">
                    <table id="users-table" class="table enhanced-sticky table-striped table-hover"style="display: table; width:100%; height: auto;">
                        <thead>
                            <tr>
    ...
    <input type="hidden" id="type" value="{{ request('user_type') }}">
    @push('scripts')
        <!-- DataTables JS -->
        ...
        <script>
            ...
            const urlIndex = `{{ route('dashboard.users.index') }}`;
            const urlFilters = `{{ route('dashboard.users.filters', ':column') }}`;
            const urlCreate = '{{ route("dashboard.users.create") }}';
            const urlShow = '{{ route("dashboard.users.show", ":id") }}';
            const urlEdit = '{{ route("dashboard.users.edit", ":id") }}';
            const urlDelete = '{{ route("dashboard.users.destroy", ":id") }}';

            // ability
            const abilityCreate = "{{ Auth::guard('admin')->user()->can('create', 'App\\Models\\User') }}";
            const abilityShow = "{{ Auth::guard('admin')->user()->can('show', 'App\\Models\\User') }}";
            const abilityEdit = "{{ Auth::guard('admin')->user()->can('update', 'App\\Models\\User') }}";
            const abilityDelete = "{{ Auth::guard('admin')->user()->can('delete', 'App\\Models\\User') }}";
            ...
            const columnsTable = [
                    ...
                    return `
                        <div style="display:flex; align-items:center; gap:8px;">
                            ...
                            <div style="display:flex; flex-direction:column;">
                                <span style="font-weight:500; color:#000;">${name}</span>
                ...
                { data: 'country', name: 'country', orderable: false, class: 'text-center' },
                { data: 'profiles', name: 'profiles', orderable: false, class: 'text-center', render: function(data, type, row) {
                    return `
                        <span style="
                            display:inline-flex;
                            align-items:center;
                            ...
                            font-weight:500;
                        ">
                            <i class="ph ph-user" style="font-size:1rem;"></i> ${data}
                        </span>
                    `;
                } },
                { data: 'is_active', name: 'is_active', orderable: false, class: 'text-center', render: function(data, type, row) {
                    if (data) {
                        return `
                            <span style="
                                display:inline-flex;
                ...
                { data: 'last_activity', name: 'last_activity', orderable: false, class: 'text-center' },
