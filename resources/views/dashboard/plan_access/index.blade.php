<x-dashboard-layout>
    ...
    <x-slot:extra_nav>
        ...
        @can('create', 'App\\Models\\PlanContentAccess')
        <div class="mx-2 nav-item">
            <a href="{{ route('dashboard.plan_access.create') }}" class="m-0 btn btn-icon text-success">
    ...
    @php
        ...
        $fields = [
            'plan_id'   =>  __('admin.sub_plans'),
            'content_type'   =>  __('admin.content_type'),
            'content_id'   =>  __('admin.content_id'),
            'access_type'   =>  __('admin.access_type'),
        ];
    @endphp
    ...
    @push('scripts')
        ...
        <script>
            ...
            const urlIndex   = `{{ route('dashboard.plan_access.index') }}`;
            const urlFilters = `{{ route('dashboard.plan_access.filters', ':column') }}`; // ← صححنا اسم الراوت
            const urlCreate  = '{{ route("dashboard.plan_access.create") }}';
            const urlEdit    = '{{ route("dashboard.plan_access.edit", ":id") }}';
            const urlDelete  = '{{ route("dashboard.plan_access.destroy", ":id") }}';

            // ability
            const abilityCreate = "{{ Auth::guard('admin')->user()->can('create', 'App\\Models\\PlanContentAccess') }}";
            const abilityShow   = "{{ Auth::guard('admin')->user()->can('show',   'App\\Models\\PlanContentAccess') }}";
            const abilityEdit   = "{{ Auth::guard('admin')->user()->can('update', 'App\\Models\\PlanContentAccess') }}";
            const abilityDelete = "{{ Auth::guard('admin')->user()->can('delete', 'App\\Models\\PlanContentAccess') }}";

            // أسماء الحقول للفلترة في الهيدر
            const fields = ['#','plan_id','content_type','content_id','access_type'];
            ...
            const columnsTable = [
                ...
                { data: 'plan_id', name: 'plan_id', orderable:false, render: (d)=> d ?? '' },
                { data: 'content_type', name: 'content_type', orderable:false, render: (d)=> d ?? '' },
                { data: 'content_id', name: 'content_id', orderable:false, render: (d)=> d ?? '' },
                { data: 'access_type', name: 'access_type', orderable:false, render: (d)=> d ?? '' },
