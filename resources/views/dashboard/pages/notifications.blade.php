<x-dashboard-layout>
    ...
    <div class="shadow-lg enhanced-card">
        <div class="table-header-title">
            <i class="icon ph ph-notifications me-2"></i>
            جدول الاشعارات
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label fw-bold">نوع الإشعار:</label>
                            <div id="notif-type">---</div>
                        </div>
                        <div class="col-md-6">
                        ...
                        <div class="col-12">
                            ...
                            <div id="notif-message" style="white-space: pre-wrap;">---</div>
                        </div>
                        <div class="col-md-6">
    ...
    @push('scripts')
        ...
        <script>
            ...
            const urlIndex = `{{ route('dashboard.notifications.index') }}`;
            const urlFilters = `{{ route('dashboard.notifications.filters', ':column') }}`;
            const urlCreate = '';
            const urlShow = `{{ route('dashboard.notifications.show', ':id') }}`;
            const urlEdit = '';
            const urlDelete = `{{ route('dashboard.notifications.destroy', ':id') }}`;

            // ability
            const abilityCreate = "{{ Auth::guard('admin')->user()->can('create', 'App\\Models\\Notification') }}";
            const abilityShow = "{{ Auth::guard('admin')->user()->can('show', 'App\\Models\\Notification') }}";
            const abilityEdit = "{{ Auth::guard('admin')->user()->can('update', 'App\\Models\\Notification') }}";
            const abilityDelete = "{{ Auth::guard('admin')->user()->can('delete', 'App\\Models\\Notification') }}";
            ...
            const columnsTable = [{
                ...
                }, // عمود الترقيم التلقائي
                {
                    ...
                    render: function(data, type, row) {
                        return `<div style="white-space: normal; text-align: right;">${data}</div>`;
                    }
                },
                {
                    data: 'type',
...

                {
                    ...
                    render: function(data, type, row) {
                        if (data) {
                            ...
                            return `<span class="badge bg-warning text-dark">غير مقروء</span>`;
                        }
                    }
                },
...

                {
                    ...
                    render: function(data, type, row) {
                        ...
                        if (abilityShow) {
                            linkshow = `
                            <button class="action-btn btn-show" data-id="${data}" title="عرض">
        ...
        <script>
            $(document).on('click', '.btn-show', function () {
                ...
                $.get(`${urlShow.replace(':id', id)}`, function(response) {
                    // تأكد من أن response يحتوي على الحقول المطلوبة
                    $('#notif-type').text(response.type ?? '-');
                    $('#notif-status').html(response.read_at ?
                        '<span class="badge bg-success">مقروء</span>' :
                        '<span class="badge bg-warning text-dark">غير مقروء</span>'
                    );
                    $('#notif-message').text(response.message ?? '-');
                    $('#notif-date').text(response.created_at ?? '-');
                });
            });
