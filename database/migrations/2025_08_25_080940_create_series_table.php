...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('series', function (Blueprint $table) {
            ...
            $table->date('first_air_date')->nullable(); // تاريخ أول عرض
            $table->date('last_air_date')->nullable(); // تاريخ آخر عرض
            $table->integer('seasons_count')->default(0); // عدد المواسم
            $table->integer('episodes_count')->default(0); // عدد الحلقات الإجمالي
            $table->decimal('imdb_rating', 3, 1)->nullable(); // تقييم IMDb
            $table->string('content_rating', 10)->nullable(); // التصنيف العمري
            $table->string('language', 5)->default('ar'); // لغة المسلسل
            $table->string('country', 2)->nullable(); // بلد الإنتاج
            $table->enum('status', ['draft', 'published', 'archived'])->default('draft'); // حالة النشر
            $table->enum('series_status', ['returning', 'ended', 'canceled'])->nullable(); // حالة المسلسل
